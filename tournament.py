"""Estimate the strength rating of our custom defined heuristic by competing
against fixed-depth minimax and alpha-beta search agents in a round-robin
tournament.

Our agent plays a number of "fair" matches against each test agent.
...
game_agent.py. Wait and let the agents play these matches.
"""
...
def play_matches(cpu_agents, test_agents, num_matches):
    ...
    if total_timeouts:
        print(("\nThere were {} timeouts during the tournament -- make sure " +
               "the agent handles search timeout correctly, and consider " +
               "increasing the timeout margin for the agent.\n").format(
            total_timeouts))
    if total_forfeits:
        print(("\nThe ID search forfeited {} games while there were still " +
